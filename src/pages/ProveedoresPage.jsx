...
import LectorFactura from '../components/modals/LectorFactura'
...
export default function ProveedoresPage() {
  ...
  const [facData, setFacData] = useState({ provId:'', num:'', fec:todayStr(), ven:'', not:'' })
  ...
  function openNewFac() {
    ...
    setFacData({ provId: proveedores[0]?.id || '', num:'', fec:todayStr(), ven:'', not:'' })
    setFotoFile(null); setFotoPreview(null)
    setModalFac(true)
  }

  // Cuando el lector extrae datos, los usa para rellenar la factura
  function onDatosExtraidos(datos) {
    if (datos.items?.length > 0) {
      setLineasFac(datos.items.map(i => ({ nombre: i.nombre, cantidad: i.cantidad, precio_unit: i.precio_unit })))
    }
    if (datos.numeroFactura) setFacData(f => ({ ...f, num: datos.numeroFactura }))
    if (datos.fecha) setFacData(f => ({ ...f, fec: datos.fecha }))
    // Intentar encontrar el proveedor por nombre
    if (datos.proveedor) {
      const prov = proveedores.find(p => p.nombre.toLowerCase().includes(datos.proveedor.toLowerCase().slice(0, 5)))
      if (prov) setFacData(f => ({ ...f, provId: prov.id }))
    }
    toast.success('Datos cargados en la factura. Revisa y guarda.')
    setModalFac(true)
  }
  ...
  async function saveFac() {
    if (!facData.provId) { toast.error('Selecciona un proveedor'); return }
    const prov = proveedores.find(p => p.id === facData.provId)
    ...
    try {
      ...
      if (fotoFile) {
        ...
        const path = `${facData.provId}/${Date.now()}.${ext}`
      ...
      await supabase.from('facturas').insert({
        proveedor_id: facData.provId,
        proveedor_nombre: prov?.nombre || '',
        numero_factura: facData.num,
        fecha_factura: facData.fec,
        fecha_vencimiento: facData.ven || null,
        items: lineasFac,
        total,
        foto_url: fotoUrl,
        notas: facData.not,
  ...
  return (
    <div className="fade-in">
              ...
              <tbody>
                {proveedores.map(p => (
                  <tr key={p.id} style={{ borderBottom:'1px solid var(--bor)' }}>
                    <td style={{ padding:'9px 11px' }}><strong>{p.nombre}</strong></td>
                    ...
                    <td style={{ padding:'9px 11px' }}>
                      {isAdmin && <div style={{ display:'flex',gap:5 }}>
                        <Btn variant="ghost" size="sm" onClick={() => openEditProv(p)}>Editar</Btn>
                        <Btn variant="danger" size="sm" onClick={() => delProv(p.id,p.nombre)}>Borrar</Btn>
                      </div>}
                    </td>
                  </tr>
                ))}
      ...
      <Card>
        <CardTitle>
          ...
          <div style={{ display:'flex',gap:7 }}>
            <Btn variant="info" size="sm" onClick={() => setModalLector(true)}>📷 Leer factura con IA</Btn>
        ...
        {loadingF ? <Loading /> : facturas.length === 0 ? <Empty icon="🧾" text="No hay facturas"/> : facturas.map(f => (
          <div key={f.id} style={{ background:'var(--sur)',border:'1px solid var(--bor)',borderRadius:'var(--r)',padding:13,marginBottom:7,cursor:'pointer' }}
            ...
            <div style={{ fontSize:11,color:'var(--txt3)' }}>
              {f.fecha_factura}{f.foto_url && ' · Con foto'}
            </div>
      ...
      {modalLector && (
        <LectorFactura
          onDatosExtraidos={onDatosExtraidos}
          onClose={() => setModalLector(false)}
        />
      )}

      {/* Modal proveedor */}
      <Modal open={modalProv} onClose={()=>setModalProv(false)} title={editProvId?'Editar proveedor':'Nuevo proveedor'}>
        <Grid2>
          <FormGroup label="Nombre *"><input value={formProv.nombre} onChange={e=>setFormProv(f=>({...f,nombre:e.target.value}))} autoFocus/></FormGroup>
        ...
        <Grid2>
          ...
          <FormGroup label="NIF"><input value={formProv.nif} onChange={e=>setFormProv(f=>({...f,nif:e.target.value}))}/></FormGroup>
        </Grid2>
      ...
      <Modal open={modalFac} onClose={()=>setModalFac(false)} title="Nueva factura de proveedor" wide>
        <Grid2>
          <FormGroup label="Proveedor *">
            <select value={facData.provId} onChange={e=>setFacData(f=>({...f,provId:e.target.value}))}>
              <option value="">Selecciona...</option>
              {proveedores.map(p=><option key={p.id} value={p.id}>{p.nombre}</option>)}
            </select>
          </FormGroup>
          <FormGroup label="N° Factura"><input value={facData.num} onChange={e=>setFacData(f=>({...f,num:e.target.value}))} placeholder="F-2024-001"/></FormGroup>
        </Grid2>
        <Grid2>
          <FormGroup label="Fecha"><input type="date" value={facData.fec} onChange={e=>setFacData(f=>({...f,fec:e.target.value}))}/></FormGroup>
          <FormGroup label="Vencimiento"><input type="date" value={facData.ven} onChange={e=>setFacData(f=>({...f,ven:e.target.value}))}/></FormGroup>
        </Grid2>
        <FormGroup label="Foto de la factura">
          <input type="file" accept="image/*,application/pdf" onChange={handleFoto} style={{ padding:8 }}/>
          {fotoPreview && !fotoPreview.isPdf && <img src={fotoPreview.url} alt="preview" style={{ width:'100%',maxHeight:160,objectFit:'cover',borderRadius:'var(--r)',marginTop:7 }}/>}
        </FormGroup>
        ...
        {lineasFac.map((l,i) => (
          <div key={i} style={{ display:'flex',gap:5,marginBottom:5,alignItems:'center',flexWrap:'wrap' }}>
            <input type="text" value={l.nombre} placeholder="Producto" style={{ flex:2,minWidth:120 }} onChange={e=>updateLinea(i,'nombre',e.target.value)}/>
            ...
            <Btn variant="danger" size="sm" onClick={()=>setLineasFac(ls=>ls.filter((_,idx)=>idx!==i))}>x</Btn>
          </div>
        ))}
        ...
        <FormGroup label="Notas" style={{ marginTop:9 }}><textarea value={facData.not} onChange={e=>setFacData(f=>({...f,not:e.target.value}))} rows={2}/></FormGroup>
        <ModalFooter>
      ...
      {modalVerFac && (
        <Modal open title="Detalle de factura" onClose={()=>setModalVerFac(null)}>
          ...
          {modalVerFac.foto_url && <img src={modalVerFac.foto_url} alt="factura" style={{ width:'100%',maxHeight:200,objectFit:'cover',borderRadius:'var(--r)',marginBottom:10 }}/>}
          {(modalVerFac.items||[]).map((i,idx) => (
            <div key={idx} style={{ display:'flex',alignItems:'center',gap:8,padding:'5px 0',borderBottom:'1px solid var(--bor)',fontSize:12 }}>
              ...
              <span style={{ color:'var(--pur)',fontWeight:600 }}>{fmt(i.precio_unit)}/ud</span>
            </div>
          ))}
